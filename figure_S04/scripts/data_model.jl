...
function induction_time(timePoints::Vector{Float64}, surfaceTension::Vector{Float64}, y0::Float64, drop::Float64)
	...
	#curve = denoise(surfaceTension; factor = 1.0)[1] # Model
...
function meso_equilibrium(timePoints::Vector{Float64}, surfaceTension::Vector{Float64}, slope::Float64)
	...
	#curve = denoise(surfaceTension; factor = 4.0)[1] # Model
	curve = surfaceTension # Model
...
uniqueConcentrations = unique([read_data_from_file("data_model/"*fileName)[1] for fileName in fileNames])
...
for i in eachindex(fileNames)
	...
	# end
	push!(tinds, (concentration, t, v, fileNames[i])) # Model
